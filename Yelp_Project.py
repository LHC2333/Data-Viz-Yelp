...
new_data = open_data.where(categoricaldata).dropna(thresh = 18).sort_values(by=['stars'],ascending=False)
...
bar1 = figure(title="Overall Rating within this Area",tools="save",
...
layout = row(widgetbox(text,search,select_day), column(plot,row(bar1,pie1)))
